...
You can follow the following steps to create AClass object or skip to part 2 to load the datd.
```{r eval=FALSE}
library(GEOquery)
...
This workflow can be adapted for any tabular gene expression input, as long as genes are in columns, samples in rows, and subgroup labels are in a `Group` column. For RNA-seq data, ensure proper normalization (e.g., TPM or log2 CPM) before use.
